...
from nxTomo_normalizer import extract_flat_corrected_sinograms

if __name__ == "__main__":
    usage = "%prog [options] input_nexus output_directory"
    parser = optparse.OptionParser(usage=usage, version="%prog 1.0")
    parser.add_option("-b", "--begin", dest="begin", help="sinogram to begin from", default=0, type='int')
    parser.add_option("-s", "--slices", dest="slices", help="number of sinograms to process", default=-1, type='int')
    parser.add_option("-f", "--format", dest="format", help="format for the output files", default="s_%05d.tif")
    ...
    for i in range(options.begin, options.begin + options.slices, options.num_slices):
        ...
        data = extract_flat_corrected_sinograms(args[0], i, end)
